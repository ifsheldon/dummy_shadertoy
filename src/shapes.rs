use crate::data::{Length, Minus, Vec3, VecDot};
...
pub fn sdf_ellipsoid(ref_pos: &Vec3, dimensions: &Vec3) -> f32 {
    let k0v = Vec3::new_xyz(
        ref_pos.x() / dimensions.x(),
        ref_pos.y() / dimensions.y(),
        ref_pos.z() / dimensions.z(),
    );
    let k0 = k0v.get_length();
    let k1v = Vec3::new_xyz(
        ref_pos.x() / (dimensions.x() * dimensions.x()),
...
pub fn sdf_rounded_cylinder(ref_pos: &Vec3, radius: f32, round_radius: f32, height: f32) -> f32 {
    let d1 =
        (ref_pos.x() * ref_pos.x() + ref_pos.z() * ref_pos.z()).sqrt() - 2. * radius + round_radius;
    let d2 = ref_pos.y().abs() - height;
    let d1_max = d1.max(0.);
    let d2_max = d2.max(0.);
    return f32::min(f32::max(d1, d2), 0.) + (d1_max * d1_max + d2_max * d2_max).sqrt()
        - round_radius;
}
